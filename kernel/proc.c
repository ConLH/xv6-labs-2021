...
// Allocate a page for each process's kernel stack. 为每个进程的内核堆栈分配一个页面。
// Map it high in memory, followed by an invalid 将其映射到内存的高位，然后是无效的保护页。
// guard page.
void
proc_mapstacks(pagetable_t kpgtbl) {
  ...
  for(p = proc; p < &proc[NPROC]; p++) {
    ...
    if(pa == 0)
      panic("kalloc");
    uint64 va = KSTACK((int) (p - proc));//获取元素间隔
...
// Grow or shrink user memory by n bytes. 减少或增加用户内存
